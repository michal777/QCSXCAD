...
void PageMaterials::SaveToSimScriptBuffer(void)
{
    ...
    for(int i_mat = 0; i_mat < materials_param_list_ptr->count(); ++i_mat)
    {
        if(materials_param_list_ptr->at(i_mat).type == "material")
        {
            text_save_to_simscript.append(QString("CSX = AddMaterial(CSX, '%1');\n").arg(materials_param_list_ptr->at(i_mat).name));

            if(materials_param_list_ptr->at(i_mat).epsilon != "" || materials_param_list_ptr->at(i_mat).mue != "" || materials_param_list_ptr->at(i_mat).kappa != "" || materials_param_list_ptr->at(i_mat).sigma != "")
            {
                text_save_to_simscript.append(QString("CSX = SetMaterialProperty(CSX, '%1'").arg(materials_param_list_ptr->at(i_mat).name));
                if(materials_param_list_ptr->at(i_mat).epsilon != "")
                    text_save_to_simscript.append(QString(", 'Epsilon', %1").arg(materials_param_list_ptr->at(i_mat).epsilon));
                if(materials_param_list_ptr->at(i_mat).mue != "")
                    text_save_to_simscript.append(QString(", 'Mue', %1").arg(materials_param_list_ptr->at(i_mat).mue));
                if(materials_param_list_ptr->at(i_mat).kappa != "")
                    text_save_to_simscript.append(QString(", 'Kappa', %1").arg(materials_param_list_ptr->at(i_mat).kappa));
                if(materials_param_list_ptr->at(i_mat).sigma != "")
                    text_save_to_simscript.append(QString(", 'Sigma', %1").arg(materials_param_list_ptr->at(i_mat).sigma));
                text_save_to_simscript.append(QString(");\n").arg(materials_param_list_ptr->at(i_mat).sigma));
            }
        }
        else if(materials_param_list_ptr->at(i_mat).type == "metal")
        {
            if(materials_param_list_ptr->at(i_mat).conductivity == "" || materials_param_list_ptr->at(i_mat).thickness == "")
                text_save_to_simscript.append(QString("CSX = AddMetal(CSX, '%1');\n").arg(materials_param_list_ptr->at(i_mat).name));
            else
                text_save_to_simscript.append(QString("CSX = AddConductingSheet(CSX, '%1', %2, %3);\n").arg(materials_param_list_ptr->at(i_mat).name).arg(materials_param_list_ptr->at(i_mat).conductivity).arg(materials_param_list_ptr->at(i_mat).thickness));
        }
    }
